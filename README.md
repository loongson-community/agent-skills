# 龙芯开源社区（非官方）维护之智能体技能库

## 许可证

为鼓励知识共享，本项目的内容在以下的开源许可证下公开发布：

- 代码部分：[`AGPL-3.0-or-later`](./LICENSE.AGPL-3.txt)
- 文档部分：[`CC-BY-NC-SA 4.0`](./LICENSE.CC-BY-NC-SA-4.0.txt)

## 人工智能生成内容（AIGC）政策

本项目本身就是为了更高效地引导智能体工作，因此在本项目拒绝 AIGC 在逻辑上是不自洽的。但考虑到当下围绕
AIGC 知识产权的讨论方兴未艾，我们认为有必要对 AIGC 做一定的考虑与区隔。

### 提交拆分与标记

本项目对人类产出与 AI 产出的内容实施严格的区分与标记。此处“人类”指法律意义上的自然人。

- 本项目的每个 Git 提交的内容，要么完全由人类撰写，要么完全由 AI 撰写。
  - 如您意欲修改一部分 AIGC 并将其作为一个整体提交，请注意：这将使整个提交内容被视作您的创作。
- 任何提交的全部内容必须由人类提交者复核。
  - 该人类须在提交说明的最后（trailer）部分附加 `Signed-off-by` 标签，即 DCO。
  - 按照现行的法律理解与实践，为某提交标注了 DCO 信息的人类，可能会被认为对该提交的全部内容负责。
  - 您作为自然人，如果对一些内容，您自身无法完全理解并接受（如果交给您自己做，您也会这么做），那么就建议您不要提交这些内容。
- 如人类认为有必要修改 AI 输出的内容，鉴于上文所述的法律后果，请先让 AI 完成提交，再自己上场修改。
- 如人类认为 AIGC 的绝大部分都需要重做，人类可以先让 AI 完成提交，也可以直接撤回此轮的 AIGC。

但有些时候受限于能力，AI 无论如何都无法遵守指示撰写提交说明，此时允许人类在 AI 生成的提交说明最后添加一段形如这样的文字：

```markdown
Human note: Something.
Something else. You can use either English or Chinese.

一个新的段落。您可自由使用中文或英文。
```

以对 AI 生成的内容作必要解释。

### AIGC 的范围界定

本政策所指的 AIGC 包括但不限于以下类型的内容，只要其主要由 AI 系统生成：

- 代码（含测试、构建脚本、配置文件等）
- 文档与注释
- 翻译
- 提交说明（commit message）

纯粹由人类编写、仅经 AI 辅助进行拼写检查或格式化的内容，不视为 AIGC。

### AI 工具与模型信息记录

为提高可追溯性，建议（暂不强制要求）在 AIGC 提交中记录所使用的 AI 工具信息。推荐的做法是在提交说明的 trailer 部分添加如下标签：

```
AI-tool: <工具名称> (<模型名称/版本>)
```

例如：

```
AI-tool: GitHub Copilot (Claude Opus 4.6)
AI-tool: Cursor (GPT-4o)
```

### 知识产权来源与许可证合规

- AI 输出的内容不得包含明显来自非兼容许可证项目的代码片段。提交者（即标注了 DCO 的自然人）有义务对此进行合理审查。
- 如 AI 输出的内容明显引用或改编自已知的外部作品，提交者应当注明出处。
- AI 输出的内容一经提交至本项目，即视为按照本项目的许可证条款（代码部分 AGPL-3.0-or-later，文档部分 CC-BY-NC-SA 4.0）进行授权。

### 人类复核的最低要求

"任何提交的全部内容必须由人类提交者复核"的具体要求如下：

- 提交者须已阅读并理解提交的全部内容。
- 提交者须确认内容的技术正确性在其能力范围内是可接受的。
- 提交者须确认内容不包含敏感信息（如密钥、凭据、个人隐私数据等）。
- 提交者须确认内容不违反本项目的许可证条款。

### 违规处理

- 如发现提交的 AIGC 标记缺失或不正确，维护者应当要求贡献者通过 `git rebase` 等方式修正提交历史，或在后续提交中补充说明。
- 如发现提交的内容存在许可证合规问题，维护者可要求回退（revert）相关提交。
- 最终裁量权归项目维护者所有。
